...
{
    public static void DrawList(SerializedProperty self, Component_Gimmick target)
    {
        ...
        using (new EditorGUI.IndentLevelScope(1))
        {
            ...
            for (var i = 0; i < self.arraySize; i++)
            {
                ...
                {
                    string s = i < target.Comp.Count ? target.Comp[i].type.ToString() : "";
                    EditorGUILayout.PropertyField(prop, new GUIContent(i.ToString() +":"+ s)
